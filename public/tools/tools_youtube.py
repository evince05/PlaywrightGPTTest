...
def tool_youtube_search(_controller: PlaywrightController, args: Dict[str, Any]) -> Dict[str, Any]:
	...
	try:
		...
		try:
			page.wait_for_selector("ytd-video-renderer,ytd-grid-video-renderer", timeout=8000)
		except Exception:
			pass
...
def tool_youtube_watch(_controller: PlaywrightController, args: Dict[str, Any]) -> Dict[str, Any]:
	...
	try:
		...
		first = page.query_selector("ytd-video-renderer a#video-title,ytd-grid-video-renderer a#video-title")
		if not first:
			# fallback: attempt to extract href via evaluate
			href = page.evaluate("() => { const a = document.querySelector('ytd-video-renderer a#video-title, ytd-grid-video-renderer a#video-title'); return a ? a.href : null; }")
			if not href:
		...
		return _result_ok({"status": "playing", "requested": title, "title": title_actual, "url": current})
	except Exception as e:
		return _result_error(e)


def tool_youtube_like(_controller: PlaywrightController, args: Dict[str, Any]) -> Dict[str, Any]:
...
def tool_youtube_subscribe(_controller: PlaywrightController, args: Dict[str, Any]) -> Dict[str, Any]:
...
def tool_youtube_pause_play(_controller: PlaywrightController, args: Dict[str, Any]) -> Dict[str, Any]:
...
_YOUTUBE_TOOL_SPECS = {
	"yt_search": ToolSpec(
		...
		parameters={"type": "object", "properties": {"query": {"type": "string"}, "timeout": {"type": "integer"}}},
	),
	"yt_watch": ToolSpec(
		name="yt_watch",
		description="Search YouTube for a video by title and open the first result.",
		parameters={"type": "object", "properties": {"title": {"type": "string"}, "query": {"type": "string"}, "timeout": {"type": "integer"}}},
	),
	"yt_like": ToolSpec(
		name="yt_like",
